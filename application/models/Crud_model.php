...
class Crud_model extends CI_Model {
    ...
    public function add_course($param1 = "") {
        ...
        $data['course_slug'] = strtolower( str_replace(" ", "-", html_escape($this->input->post('title'))  ));
        ...
        if (!file_exists('uploads/overviews/course_overview'.$course_id.'/')) {
            mkdir('uploads/overviews/course_overview/'.$course_id.'/', 0777, true);
        }
        ...
        // print_result('uploads/overviews/course_overview/'.$course_id.'/'.$_FILES['course_overview_url']['name']);
        if ($_FILES['course_overview_url']['name'] != "") {
            move_uploaded_file($_FILES['course_overview_url']['tmp_name'], 'uploads/overviews/course_overview/'.$course_id.'/'.$_FILES['course_overview_url']['name'] );
        }
        $updateData =  array(
            'video_url' => 'uploads/overviews/course_overview/'.$course_id.'/'.$_FILES['course_overview_url']['name']
             );
        $whereConditionArray = array(
            'id' =>$course_id );
        $this->db->where($whereConditionArray);
        $this->db->update('course', $updateData);
        // print_result(';ghere');
    ...
    public function get_courses_by_category_id($category_id) {
        $this->db->select("*");
    ...
    public function add_lesson() {
        ...
        // video
        if (!file_exists('uploads/lessons_videos/'.$inserted_id.'/')) {
            mkdir('uploads/lessons_videos/'.$inserted_id.'/', 0777, true);
        }

        if ($_FILES['html5_video_url']['name'] != "") {
            $fileName = strtolower(str_replace(" ", "_", $_FILES['html5_video_url']['name']  ));
            move_uploaded_file($_FILES['html5_video_url']['tmp_name'], 'uploads/lessons_videos/'.$inserted_id.'/'.$fileName );
        
        $data['video_url'] = html_escape($this->input->post('html5_video_url'));

        $updateData =  array(
            'video_url' => 'uploads/lessons_videos/'.$inserted_id.'/'.$fileName
             );
        $whereConditionArray = array(
            'id' =>$inserted_id );
        $this->db->where($whereConditionArray);
        $this->db->update('lesson', $updateData);
        }
        // end video
    }
    ...
    public function get_load_more_course($limit,$start) {
        $this->db->order_by("id", "desc");
    ...
    public function enrol_student_react($user_id){
            $purchased_course = $this->input->post('course_id');
            // print_result($purchased_course);
            $data['user_id'] = $user_id;
            $data['course_id'] = $purchased_course;
            $data['date_added'] = strtotime(date('D, d-M-Y'));
            $this->db->insert('enrol', $data);
    ...
    public function course_purchase_resct($user_id, $method, $amount_paid,$course_id) {
